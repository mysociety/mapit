...
class Command(LabelCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--allow-gss-changes",
            action="store_true",
            dest="allow_gss_changes",
            help="Allow CED areas to have their GSS codes changed (e.g. boundary redrawn)",
        )
        parser.add_argument(
            "--generation",
            dest="generation",
    ...
    def handle_label(self, filename, **options):
        ...
        for feat in layer:
            ...
            area, overlap = self._best_match(geometry, generation)
            if area is None:
            ...
            if options["allow_gss_changes"]:  # Overwrite whatever GSS code might already be there
                area.codes.update_or_create(type=gss, defaults={"code": ons_code})
            else:
                try:
                    area.codes.get_or_create(type=gss, code=ons_code)
                except IntegrityError:  # There's already a GSS code for this area and we've tried to add a different one
                    old_code = area.codes.get(type=gss).code
                    self.stderr.write(f"{area.name} (ID {area.id}) has changed code? Old: {old_code} New: {ons_code}. Overlap: {overlap:.2f}%")
                    if options["commit"]:
                        raise
                    else:  # on a dry-run let it proceed so we can evaluate all errors
                        continue
    ...
    def _best_match(self, geometry, generation):
        """Return the existing CED Area whose polygons overlap geometry the
        most, or None if MIN_OVERLAP isn't met, as well as a percentage of how
        well this overlaps existing area."""
        ...
        if best_overlap < MIN_OVERLAP * geometry.area:
            return (None, (best_overlap / geometry.area) * 100)
        return (best, (best_overlap / geometry.area) * 100)
