...
class Command(NoArgsCommand):
    ...
    def emit_csv(self, old_areas_filters, const_code, const_name, old_areas_names, filter_type='all', file=sys.stdout):
        ...
        if filter_type == 'all':
            ...
            r = r.exclude(name__in=old_areas_names).values_list('id', 'name')
            old_areas_csv = ",".join(["{0} {1}".format(id, name) for id, name in r])
        else:
            r = r.filter(name__in=old_areas_names).values_list('id', 'name')
            old_areas_csv = ",".join(["{0} {1}".format(id, name) for id, name in r])
